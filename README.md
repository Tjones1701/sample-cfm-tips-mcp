...
Deploy the CFM Tips MCP server to AWS Lambda with Streamable HTTP transport, fronted by an API Gateway HTTP API with IAM authentication.

### Prerequisites

- **Python 3.13** — required for the Lambda runtime and CDK dependencies
- **Node.js** (v18+) — required for `npx cdk` commands
- **AWS CLI** — configured with credentials (named profile, environment variables, or IAM role) that have permissions to deploy CloudFormation stacks, Lambda functions, and API Gateway resources
...
```bash
# Basic deploy (no cross-account role)
./deploy.sh --profile my-aws-profile

# Deploy with cross-account integration role
./deploy.sh --profile my-aws-profile --remote-account-id 123456789012

# With explicit region
./deploy.sh --profile my-aws-profile --region us-east-1 --remote-account-id 123456789012
...
The script will:
1. Create a Python virtualenv in `infra/.venv` and install CDK dependencies
2. Run `cdk bootstrap` (idempotent, safe to re-run)
3. Run `cdk deploy` with the specified profile/region (or defaults)
4. Print the stack outputs on success

If `--remote-account-id` is provided, the stack creates an `AvatarIntegration-CFMTipsMCP` IAM role that trusts the specified account. The script outputs the role ARN and a generated external ID that the remote account needs for assume-role.

If no `--region` is specified, the region is determined by your AWS CLI profile or `AWS_DEFAULT_REGION` environment variable.

### Verify

After deployment, the script prints two values:

```
API Gateway Endpoint: https://<api-id>.execute-api.<region>.amazonaws.com/
Lambda Function ARN:  arn:aws:lambda:<region>:<account-id>:function:CfmTipsMcpStack-CfmTipsMcpFunction...
```

You can also inspect `cdk-outputs.json` at the repo root:

```bash
cat cdk-outputs.json
```

The API Gateway endpoint accepts MCP JSON-RPC requests at the `/mcp` path. All requests must be signed with SigV4 (IAM auth).

## 🔗 Cross-Account IAM Configuration

When you deploy with `--remote-account-id`, the CDK stack automatically creates an `AvatarIntegration-CFMTipsMCP` IAM role in the Lambda account. This role:

- Trusts the specified remote account with an auto-generated external ID
- Has `execute-api:Invoke` permission scoped to the `/mcp` API Gateway route

After deployment, the script prints:

```
Cross-Account Integration:
  Role ARN:    arn:aws:iam::<LAMBDA_ACCOUNT_ID>:role/AvatarIntegration-CFMTipsMCP
  External ID: <auto-generated-value>
```

### What the remote account needs

The remote account already has a policy allowing it to assume roles in other accounts. The remote account's principals assume `AvatarIntegration-CFMTipsMCP` using the external ID from the deployment output:

```bash
aws sts assume-role \
  --role-arn arn:aws:iam::<LAMBDA_ACCOUNT_ID>:role/AvatarIntegration-CFMTipsMCP \
  --role-session-name cfm-tips-session \
  --external-id <EXTERNAL_ID_FROM_DEPLOY_OUTPUT>
```

### What the CDK stack creates

**AvatarIntegration-CFMTipsMCP role — permissions policy:**
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:execute-api:<REGION>:<LAMBDA_ACCOUNT_ID>:<API_ID>/*/*/mcp"
    }
  ]
}
```

**AvatarIntegration-CFMTipsMCP role — trust policy:**
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::<REMOTE_ACCOUNT_ID>:root"
      },
      "Action": "sts:AssumeRole",
      "Condition": {
        "StringEquals": {
          "sts:ExternalId": "<AUTO_GENERATED_EXTERNAL_ID>"
        }
      }
    }
  ]
...
## 🔄 End-to-End Cross-Account Flow

When a remote account invokes the MCP server, the request follows this path:

```
Remote Account                          Lambda Account
┌─────────────────────┐                ┌──────────────────────────────┐
│                     │                │                              │
│  1. Assume role     │                │  AvatarIntegration-          │
│     AvatarIntegra-  │   STS          │  CFMTipsMCP role             │
│     tion-CFMTipsMCP ├───────────────▶│  (trusts remote account)     │
│     (in Lambda acct)│   AssumeRole   │          │                   │
│          │          │                │          │                   │
│  2. Sign request    │   HTTPS +      │  3. API Gateway validates    │
│     with SigV4     ─┼──  SigV4  ────▶│     IAM authorization       │
│     (temp creds)    │                │          │                   │
│                     │                │  4. Lambda executes with     │
│                     │                │     its own execution role   │
│                     │                │          │                   │
│                     │                │  5. MCP Server processes     │
│  7. Receive JSON   ◀┼── response ───│     tool call via boto3      │
│     response        │                │          │                   │
│                     │                │  6. Returns JSON-RPC         │
│                     │                │     response                 │
└─────────────────────┘                └──────────────────────────────┘
...
1. A principal in the remote account calls `sts:AssumeRole` on `AvatarIntegration-CFMTipsMCP` (which lives in the Lambda account), providing the external ID from the deployment output
2. STS returns temporary credentials scoped to the `AvatarIntegration-CFMTipsMCP` role
3. Using those temporary credentials, the principal signs an HTTP request to the API Gateway endpoint with SigV4
4. API Gateway validates the IAM signature and confirms the caller has `execute-api:Invoke` permission
5. API Gateway forwards the request to the Lambda function via proxy integration
6. The Lambda function processes the MCP tool call using its own execution role credentials (read-only access to AWS cost APIs)
7. The response flows back through API Gateway to the caller
